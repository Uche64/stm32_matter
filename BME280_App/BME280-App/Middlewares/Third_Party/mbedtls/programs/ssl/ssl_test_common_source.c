...
#include "mbedtls/ssl.h"          // For mbedtls_ssl_context
#include "mbedtls/platform.h"     // For mbedtls_printf
#include "mbedtls/error.h"        // For error handling
#include "mbedtls/entropy.h"      // For entropy
#include "mbedtls/ctr_drbg.h"     // For random number generation
#include "mbedtls/x509_crt.h"     // For X.509 certificates

typedef struct {
    unsigned char master_secret[48];  // Master secret for TLS 1.2
    unsigned char randbytes[64];      // Client and server random values
    int tls_prf_type;                 // PRF type
} eap_tls_keys;
...
struct {
    int debug_level;
    char *nss_keylog_file;
    int nbio;        // Non-blocking I/O flag
    int transport;   // Transport type (DTLS/TLS)
} opt;


//int ret = mbedtls_net_recv((mbedtls_net_context *) ctx, buf, len);
