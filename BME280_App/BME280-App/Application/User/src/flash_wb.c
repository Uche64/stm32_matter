...
const NVM_Sector_Struct sector_no_secure = { 
        .id_sector = SECTOR_NO_SECURE, 
        .ram_ptr = ram_nvm + SECTOR_SIZE_SECURE, 
        .sector_size = SECTOR_SIZE_NO_SECURE 
        };

//*SIMULATE TO EXAMPLE*
const NVM_Sector_Struct sector_secure = { 
        .id_sector = SECTOR_SECURE, 
        .ram_ptr = ram_nvm,
        .sector_size = SECTOR_SIZE_SECURE 
        };
...
static uint8_t flash_update(const NVM_Sector_Struct select_sector, uint8_t *KeyName, 
...
NVM_StatusTypeDef NM_GetKeyValue(void *KeyValue, const char *KeyName, uint32_t KeySize, 
                                 size_t *read_by_size, NVM_Sector sector) {
...
static NVM_StatusTypeDef flash_update(const NVM_Sector_Struct select_sector, uint8_t *KeyName,
        uint8_t *KeyValue, size_t KeySize) {
...
static NVM_StatusTypeDef flash_write(uint8_t *PtKeyFree, uint8_t *key, uint8_t *value,
        size_t value_size) {
